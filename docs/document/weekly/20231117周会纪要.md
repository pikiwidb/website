...
| :-: | :-: | :-- | :-: | --- |
|  |  | [支持 ACL](https://github.com/OpenAtomFoundation/pika/issues/1284) | 谦祥 | 缺了一个 log 功能，预计本周末补上。测试用例有一个 TCL 测试在 mac 上无法通过，已经可以 Review。预计 3.5.3 带上。 |
|  |  | [支持 Redis stream](https://github.com/OpenAtomFoundation/pika/discussions/1717) | 周开颜 | 水哥代码review，3.5.3 or 3.5.4 |
...
|  |  | [Pika支持数据强一致性](https://github.com/OpenAtomFoundation/pika/issues/2073) |  | 后续跟一哥商量，是否在 Floyd 里面实现数据实时一致性 |
...
| Item | Priority | Issue/Pr | Owner | Description |
| :-: | --- | --- | :-: | --- |
|  | 马鑫外部推进，集群注册成功，rebalance 还没做，和珊珊交流做一做看 ，车金鸽、吴显荣内部推进 | [pika-operator on kubeblocks 现状总结及后续任务、思路](https://github.com/OpenAtomFoundation/pika/issues/1906) | 马鑫 | 预计下周末回出一个版本 0.7.1，有待显荣下周测试 |
