module.exports = {
    ...
    themeConfig: {
        nav: [
            ...
            // { text: '活动通知', link: '/guide/' },
            // { text: '样板案例', link: '/guide/' },
            // { text: '通用解决方案', link: '/guide/' },
            {
                text: "博客",
                items: [
                  { text: "陈宗志", link: "http://baotiao.github.io/page2/" },
                  { text: "宋昭", link: "http://kernelmaker.github.io/" },
                  { text: "吴显坚", link: "https://axlgrep.github.io/" },
                ]
              }
            // {
            //   text: "External",
        ...
        // nav: [
        //     { text: '首页', link: '/' },
        //     { text: '周会纪要', link: 'https://google.com' },
        //     // { text: '文档', link: '/guide/product-docs/' },
        //     // { text: '活动通知', link: '/guide1/' },
        //     // { text: '样板案例', link: '/guide/product-docs/' },
        //     // { text: '通用解决方案', link: '/guide/product-docs/' },
        //     // {
        //     //     text: '学习',
        //     //     items: [
        //     //         {
        ...
        sidebar: {
            '/weekly/': [
                ...
                }, 
                {
                    ...
                    collapsable: false,
                    children: [
                        { title: "数据迁移 QA", path: "/weekly/20231201周会纪要"},
                        { title: "floyd存储方案", path: "/weekly/20231124周会纪要"},//自动获取README.md
                        { title: "20231117 周会纪要", path: "/weekly/20231117周会纪要"},
                        { title: "20231110 周会纪要", path: "/weekly/20231110周会纪要"}
                    ]
                }
            ...
            //         { title: "支持的语言和客户端", path: "/document/use/支持的语言和客户端"},
            //         { title: "当前支持的Redis接口以及兼容情况", path: "/document/use/当前支持的Redis接口以及兼容情况"},
            //         { title: "配置文件说明", path: "/document/use/配置文件说明"},
            //         { title: "数据目录说明", path: "/document/use/数据目录说明"},
            //         { title: "info信息说明", path: "/document/use/info信息说明"},
            //         { title: "部分管理指令说明", path: "/document/use/部分管理指令说明"},
            //         { title: "差异化命令", path: "/document/use/差异化命令"},
            //         { title: "Pika Sharding Tutorials", path: "/document/use/Pika_Sharding_Tutorials"},
            //         { title: "Pika订阅", path: "/document/use/Pika订阅"},
            //         { title: "配合sentinel(哨兵)实现pika自动容灾", path: "/document/use/配合sentinel实现pika自动容灾"},
            //         { title: "如何升级到Pika3.0", path: "/document/use/如何升级到Pika3.0"},
            //         { title: "如何升级到Pika3.1或3.2", path: "/document/use/如何升级到Pika3.1"},
            //         { title: "Pika多库版命令、参数变化参考", path: "/document/use/Pika多库版命令"},
            //         { title: "Pika分片版本命令", path: "/document/use/Pika分片版本命令"},
            //         { title: "副本一致性使用说明", path: "/document/use/副本一致性使用说明"},
            //         { title: "Pika内存使用", path: "/document/use/Pika内存使用"},
            //         { title: "Pika最佳实践", path: "/document/use/Pika最佳实践"},
            //         { title: "调整 max-write-buffer-size 优化 pika 性能 10 倍的案例", path: "/document/use/调整max-write-buffer-size"},
            //         { title: "Redis 与 Pika scan 性能对比", path: "/document/use/Redis与Pikascan性能对比"},
            //         { title: "Pika 3.5 参数优化手册", path: "/document/use/Pika3.5参数优化手册"},
            //         { title: "喜马拉雅 Redis 与 Pika 缓存使用军规", path: "/document/use/喜马拉雅Redis与Pika缓存使用军规"},
            //         { title: "如何实现 3.3.6 升级到 3.5.x", path: "/document/use/如何实现3.3.6升级到3.5.x"},
            //     ]
            // }, 
            {
                title: "性能",
            ...
            //     collapsable: false,
            //     children: [
            //         { title: "3.2.新，旧，可读三类binlog转换工具", path: "/document/tool/3.2.新旧可读三类binlog转换工具"},
            //         { title: "根据时间戳恢复数据工具", path: "/document/tool/根据时间戳恢复数据工具"},
            //         { title: "Redis到Pika迁移工具", path: "/document/tool/Redis到Pika迁移工具"},
            //         { title: "Redis请求实时copy到Pika工具", path: "/document/tool/Redis请求实时copy到Pika工具"},
            //         { title: "Pika到Pika、Redis迁移工具 Pika-Port(适用于 Pika v2.x&v3.0.x)", path: "/document/tool/Pikatool1"},
            //         { title: "Pika到Pika、Redis迁移工具 Pika-Migrage(适用于 Pika v3.2及以上版本)", path: "/document/tool/Pikatool2"},
            //         { title: "Pika的kv数据写入txt文本工具", path: "/document/tool/Pika的kv数据写入txt文本工具"},
            //         { title: "kv数据txt文本迁移Pika工具", path: "/document/tool/kv数据txt文本迁移Pika工具"},
            //         { title: "pika exporter监控工具", path: "/document/tool/pikaexporter监控工具"},
            //         { title: "codis-redis实时同步pika工具", path: "/document/tool/codis-redis实时同步pika工具"},
            //     ]
            // },
            {
                title: "Develop",
            ...
            //         { title: "nemo存储引擎数据格式", path: "/document/design/nemo存储引擎数据格式"},
            //         { title: "blackwidow存储引擎数据格式", path: "/document/design/blackwidow存储引擎数据格式"},
            //         { title: "Pika源码学习--pika的通信和线程模型", path: "/document/design/Pika源码学习study1"},
            //         { title: "Pika源码学习--pika的PubSub机制", path: "/document/design/Pika源码学习study2"},
            //         { title: "Pika源码学习--pika的命令执行框架", path: "/document/design/Pika源码学习study3"},
            //         { title: "Pika源码学习--pika和rocksdb的对接", path: "/document/design/Pika源码学习study4"},
            //         { title: "pika-NoSQL原理概述", path: "/document/design/pika-NoSQL原理概述"},
            //         { title: "pika在codis中的探索", path: "/document/design/pika在codis中的探索"},
            //         { title: "Pika 笔记", path: "/document/design/Pika笔记"},
            //         { title: "pika 主从同步原理", path: "/document/design/pika主从同步原理"},
            //         { title: "Pika 新存储结构 Floyd", path: "/document/design/Pika新存储结构Floyd"},
